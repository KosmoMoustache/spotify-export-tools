#!/usr/bin/env python3
"""Sync a Spotify CSV playlist export to a Subsonic compatible server playlist.
...
Matches you confirm interactively are logged in a shared query cache (one pair
per line, original query TAB title/artist of the server match; the same file
check.py uses). On later runs they are used as a fallback whenever a track is
...
class SyncCLI(CLI):
    ...
    def build_config(self, args) -> SyncConfig:
        return SyncConfig(
            csv_path=args.csv,
...
def interactive_resolve(
    ...
    song = common_interactive_resolve(
...
def resolve_track(
    client: SubsonicClient, cfg: Config, i: int, total: int, row: dict, cache: dict
):
    ...
    if not cfg.no_cache:
        prefix = f"[{i}/{total}]"
        song = resolve_from_cache(client, prefix, query, cache, search=client.search)
        if song:
            return song, "cache"
    # No confident match with artist: retry once using only the track name,
    # then present the results for user approval.
    print(
        f"  no confident match for '{title}' by '{artists}'; "
    ...
    if song is not None and not cfg.no_cache:
        log_cached_match(f"[{i}/{total}]", query, song, cache)
    return song, how


def resolve_target_playlist(
    client: SubsonicClient, cfg: Config, default_name: str
) -> dict:
    ...
    if cfg.playlist_id:
        ...
        if not playlist:
            raise RuntimeError(
                f"Playlist id '{cfg.playlist_id}' not found on {cfg.server.base_url}"
            )
        return playlist
...
def resolve_library_ids(cfg: Config, folders: list) -> tuple:
    ...
    if missing:
        print(
            f"Unknown library/folder: {', '.join(missing)}. "
...
def choose_libraries_interactive(folders: list):
    ...
    names = [
        next(f.get("name") for f in folders if str(f.get("id")) == sid)
        for sid in selected
    ]
...
def configure_libraries(client: SubsonicClient, cfg: Config) -> None:
    ...
    if ids:
        ...
        print(
            f"Search restricted to {len(ids)} music library/libraries: "
            f"{', '.join(names)}"
        )
...
def main() -> int:
    ...
    client = SubsonicClient(cfg.server, client_name="sync")
    try:
        ping = client.ping()
        ...
        return 1
    print(
        f"OK: connected as '{cfg.server.username}', Subsonic API version {ping.get('version', '?')}"
    )
    ...
    if target:
        ...
        print(
            f"Playlist '{target.get('name')}' (id={target.get('id')}) "
    ...
    for i, row in enumerate(rows, start=1):
        try:
            song, how = resolve_track(client, cfg, i, len(rows), row, cache)
        except AbortSync:
            print("\nAborted by user.")
            if not cfg.no_cache:
    ...
    if target:
        print(
            f"Updating playlist '{target.get('name')}' (id={target.get('id')}) "
            f"with {len(resolved_ids)} tracks in CSV order ..."
        )
        try:
            result = client.create_playlist(
                playlist_id=target["id"], song_ids=resolved_ids
            )
        except Exception as exc:
...
def _print_summary(
    ...
    manual_count: int,
    applied: bool,
    client: SubsonicClient = None,
    cache_count: int = 0,
) -> None:
    name = (
        target.get("name") if target else cfg.playlist_name or Path(cfg.csv_path).stem
    )
    lines = []
    lines.append("=" * 60)
    ...
    lines.append("=" * 60)
    lines.append(
        f"Playlist          : {name} (id={target.get('id', '?') if target else 'new'})"
    )
    lines.append(f"Server            : {cfg.server.base_url}")
    lines.append(f"CSV source        : {cfg.csv_path}")
    if client and client.music_folder_ids:
        lines.append(
            f"Libraries searched : {', '.join(client.music_folder_names or client.music_folder_ids)}"
        )
    lines.append(f"CSV tracks        : {len(rows)}")
    lines.append(
        f"Matched           : {len(resolved_ids)} "
    ...
    if target:
        ...
        lines.append(
            f"Final playlist size : {diff['new_total']} (was {diff['old_total']})"
        )
    else:
        lines.append(f"New playlist size   : {len(resolved_ids)}")
    lines.append("-" * 60)
    lines.append(
        f"Changes applied to server : {'yes' if applied else 'NO (dry-run / no change)'}"
    )
    if skipped:
